  ...
  async getProgress(userId, forceRemote = false){
    ...
    if(progress && !forceRemote){
      ...
      if(activeSb && uid && !this._bgSyncActive){
        ...
        activeSb.from('user_progress').select('*').eq('user_id', uid).maybeSingle().then(async ({ data, error }) => {
          ...
          if(!error && data){
            ...
            if(cached.points !== resolvedPoints || cached.hearts !== serverHearts || cached.streak_days !== serverStreak){
              ...
              if(Array.isArray(data.claimed_gifts)) {
                try {
                  const giftKey = `mg_claimed_gifts_${uid}`;
                  const cur = JSON.parse(localStorage.getItem(giftKey) || '[]');
                  const merged = Array.from(new Set([...cur, ...data.claimed_gifts]));
                  localStorage.setItem(giftKey, JSON.stringify(merged));
                } catch (_) {}
              }
