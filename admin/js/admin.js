...
async function loadUsers(isSilent = false) {
  ...
  try {
    ...
    allLoadedStudents = users.map(u => {
      ...
      let studentPhone = u.phone || '';
      if (!studentPhone && u.email && u.email.startsWith('phone_')) {
        studentPhone = u.email.replace('phone_', '').split('@')[0];
      }
      if (!studentPhone && u.raw_user_meta_data && u.raw_user_meta_data.phone) {
        studentPhone = u.raw_user_meta_data.phone;
      }

      let studentAuthType = u.auth_type || '';
      if (!studentAuthType) {
        if (u.email && u.email.startsWith('phone_')) {
          studentAuthType = 'phone';
        } else if (u.app_metadata?.provider === 'google' || u.raw_app_meta_data?.provider === 'google') {
          studentAuthType = 'google';
        } else {
          studentAuthType = 'email';
        }
      }
...
function viewStudentDetails(userId) {
  ...
  // ملء وعرض رقم الموبايل مع إمكانية الاتصال ومحادثة واتساب والنسخ
  const modalPhoneEl = document.getElementById('m-student-phone');
  const modalPhoneCall = document.getElementById('m-student-phone-call');
  const modalPhoneWa = document.getElementById('m-student-phone-wa');
  const btnCopyPhone = document.getElementById('btn-copy-phone');
  if (modalPhoneEl) {
    if (student.phone) {
      modalPhoneEl.textContent = student.phone;
      if (modalPhoneCall) {
        modalPhoneCall.href = 'tel:' + student.phone;
        modalPhoneCall.style.display = 'inline-block';
      }
      if (modalPhoneWa) {
        const cleanWa = student.phone.replace(/[^0-9]/g, '');
        const waNum = cleanWa.startsWith('0') ? '20' + cleanWa.substring(1) : cleanWa;
        modalPhoneWa.href = `https://wa.me/${waNum}`;
        modalPhoneWa.style.display = 'inline-block';
      }
      if (btnCopyPhone) btnCopyPhone.style.display = 'inline-flex';
    } else {
      modalPhoneEl.textContent = 'غير مسجل';
      if (modalPhoneCall) modalPhoneCall.style.display = 'none';
      if (modalPhoneWa) modalPhoneWa.style.display = 'none';
      if (btnCopyPhone) btnCopyPhone.style.display = 'none';
    }
  }

  // ملء وسيلة التسجيل
  const modalAuthTypeEl = document.getElementById('m-student-auth-type');
  if (modalAuthTypeEl) {
...
function copyStudentPhone() {
  if (!activeSelectedStudent || !activeSelectedStudent.phone) {
    toast('لا يوجد رقم موبايل مسجل');
    return;
  }
  navigator.clipboard.writeText(activeSelectedStudent.phone).then(() => {
    toast('تم نسخ رقم الموبايل بنجاح ✓');
  }).catch(() => {
    toast('تعذر نسخ الرقم', true);
  });
}
...
async function deleteNotificationEvent(eventId) {
  ...
  const isAllowed = window.currentAdminRole === 'super_admin' || window.currentAdminRole === 'admin';
  if (!isAllowed) {
    if (typeof Swal !== 'undefined') {
      Swal.fire({
        icon: 'error',
